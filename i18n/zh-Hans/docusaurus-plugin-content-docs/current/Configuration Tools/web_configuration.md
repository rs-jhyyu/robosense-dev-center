...
1. 雷达与电脑完成物理连接（航插线、网线、电源线连接正常），设备通电并正常启动。
2. 配置电脑本地 IP 与雷达同网段（默认雷达 IP：192.168.1.200，建议电脑 IP：192.168.1.102，子网掩码 255.255.255.0）。
3. 关闭防火墙及安全软件，使用 Chrome/Edge/Firefox 等主流浏览器访问。

## 2. Web 端访问操作流程

确认雷达和电脑的网络连接正常，在浏览器输入雷达 Device IP，**出厂默认地址为 192.168.1.200**；若已修改过雷达 IP，输入修改后的新 IP 地址，即可进入雷达 Web 端界面。
...
2. 确保雷达与电脑的网络连接稳定，升级过程中不可断开连接、断电或重启设备 / 电脑。
...
1. **无法访问 Web 端**：
    ...
    - 检查激光雷达供电及电机运转状态；
    - 核对电脑本地 IP 与雷达 Device IP 网段、子网掩码一致性；
